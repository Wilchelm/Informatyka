...
public class Main2Activity extends AppCompatActivity {
        ...
        public void onTextChanged(CharSequence s, int start, int before, int count) {
            if(et.getText().length()>0) {
                ...
                if (num>=1 && num<=99)
                {
                    ...
                    Toast.makeText(Main2Activity.this,getResources().getString(R.string.attentio),Toast.LENGTH_SHORT).show();
                    et.setText("");
                    zmienna = 10;
                }
            }
    ...
    private void setCard() {
        ...
        Pair<String, String> p = returnRandomFromList();
        CardItem card = new CardItem(p.first, p.second);
        avatar.setImageBitmap(card.getDrawable());
