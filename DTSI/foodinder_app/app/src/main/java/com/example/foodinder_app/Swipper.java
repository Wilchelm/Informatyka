...
public class Swipper extends AppCompatActivity {
    ...
    private ImageView jeden;
    private ImageView dwa;
    private ImageView trzy;
    private ImageView cztery;
    private ImageView piec;
    ...
    private int colorCztery = 0;
    private int colorPiec = 0;
    private int green = Color.rgb(106,193,89);
    private int red = Color.rgb(239,82,39);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnLove = (ImageView) findViewById(R.id.love);
        jeden = (ImageView) findViewById(R.id.jeden);
        dwa = (ImageView) findViewById(R.id.dwa);
        trzy = (ImageView) findViewById(R.id.trzy);
        cztery = (ImageView) findViewById(R.id.cztery);
        piec = (ImageView) findViewById(R.id.piec);
        ...
        cardStack.setListener(new SwipeStack.SwipeStackListener() {
            ...
            public void onViewSwipedToLeft(int position) {
                ...
                if (i==j) {
                    i=0;
                    Intent intent = new Intent(Swipper.this, Main3Activity.class);
                    startActivity(intent);
                    finish();
                }
                i+=1;
                changeImageStack(green);
            }
    ...
    private void setCardStackAdapter() { parseJSON(); }

    private void changeImageStack( int color ) {
        int size = cardsAdapter.getCount();
        Bitmap bitmapJeden = null;
        Bitmap bitmapDwa = null;
        Bitmap bitmapTrzy = null;
        Bitmap bitmapCztery = null;
        Bitmap bitmapPiec = null;
        colorPiec = colorCztery;
        colorCztery = colorTrzy;
        colorTrzy = colorDwa;
        colorDwa = colorJeden;
        colorJeden = color;
        try {
            bitmapJeden = cardsAdapter.getItem(size - 1).getDrawable();
            jeden.setImageBitmap(bitmapJeden);
            jeden.setBackgroundColor(colorJeden);
        } catch (Exception e) {

        }
        try {
            bitmapDwa = cardsAdapter.getItem(size - 2).getDrawable();
            dwa.setImageBitmap(bitmapDwa);
            dwa.setBackgroundColor(colorDwa);
        } catch (Exception e) {

        }
        try {
            bitmapTrzy = cardsAdapter.getItem(size - 3).getDrawable();
            trzy.setImageBitmap(bitmapTrzy);
            trzy.setBackgroundColor(colorTrzy);
        } catch (Exception e) {

        }
        try {
            bitmapCztery = cardsAdapter.getItem(size - 4).getDrawable();
            cztery.setImageBitmap(bitmapCztery);
            cztery.setBackgroundColor(colorCztery);
        } catch (Exception e) {

        }
        try {
            bitmapPiec = cardsAdapter.getItem(size - 5).getDrawable();
            piec.setImageBitmap(bitmapPiec);
            piec.setBackgroundColor(colorPiec);
        } catch (Exception e) {
                ...
                new Response.Listener<JSONObject>() {
                    ...
                    public void onResponse(JSONObject response) {
                        try {
                            ...
                            cardItems.add(new CardItem(photo,name));

                            cardsAdapter = new CardsAdapter(Swipper.this, cardItems);
    ...
    private void green(){
        String url = api + "/exec?action=green";
    ...
    private void red(){
        String url = api + "/exec?action=red";
