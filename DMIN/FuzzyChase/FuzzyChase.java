...
class DrawingTask extends TimerTask {
    ...
    DrawingTask(JPanel panel) throws IOException{
        ...
        tab_speed=new double[] { 0.001, 0.002, 0.003, 0.005, 0.008, 0.01, 0.012, 0.015 }; 	//Mozliwe predkosci z jakimi porusza sie samochod 1 (x10000)
        tab_interval=new int[] { 1000, 2000, 3000, 4000 };	  				//Czas jak dlugo porusza sie z dana predkoscia samochod 1 
...
//------------>  samochod 1 - losowanie predkosci  <------------\\
		...
		speed1 = tab_speed[speed_idx]; // predkosc samochodu 1
...
//------------>  samochod 2 - sterownik rozmyty  <------------\\

		
	// parametry wejsciowe sterownika:
	// predkosc:  (speed2*10000)  w km/h
	// dystans: (phi1-phi2)*100 w metrach
	// wartosc zwracana:
	// speed_change w procentach
	...
	//speed_change = ...  <--- tu pobierz wartosc ze sterownika
	speed_change=fis.getVariable("acceleration").getValue();
	
	System.out.println(String.format("S1: %8.4f\t  S2: %8.4f\t D:%8.4f\t => P:%8.4f", speed1*10000, speed2*10000, (phi1-phi2)*100, speed_change)); //Wyswietla parametry na konsoli 
	
	speed2 = speed2 + speed2*(speed_change/100); // nowa predkosc samochodu 2
