...
/*zadanie1*/
Select nazwisko, stanowisko From Pracownicy Where id_dzialu >ALL 
(select id_dzialu from Stanowiska where id_dzialu!=20)
/*Zadanie 2*/
Select nazwisko from Pracownicy where zatrudniony>ALL
(select zatrudniony from Pracownicy where id_dzialu=20 )
/*zadanie 3*/
select nazwisko, 'Tak' as 'zarabiawiecej' from Pracownicy where nazwisko='BRZEZINSKI' AND placa_pod>SOME
(select placa_pod from Pracownicy where stanowisko='PROFESOR')
/*zadanie 4*/
select nazwisko from Pracownicy where stanowisko=SOME
(select stanowisko from Pracownicy where nazwisko='HAPKE')
/*zadanie 5*/
select nazwisko as 'Szefowie'from pracownicy where id_prac=some
(select id_prac from pracownicy where id_prac=szef)
